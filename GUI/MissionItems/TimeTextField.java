...
public class TimeTextField extends TextField {  
    ...
    public TimeTextField(String time) {  
      super(removeMiliseconds(time));  
      
      timePattern = Pattern.compile("\\d\\d:\\d\\d:\\d\\d");  
      if (! validate(removeMiliseconds(time))) {  
    ...
    private static String removeMiliseconds(String time){
    	if(time.contains(".")){//if there are miliseconds
